...
public class StreamingService {
	...
	private class Track {
		...
		public int type, encoder, format, id, port, ssrc;
	...
	public int addH264Track(int videoSource, int destinationPort, String[] params, VideoQuality videoQuality, SurfaceHolder surfaceHolder) {
		...
		int id = generateId();
		Track track = new Track(new MediaStreamer(),Track.VIDEO);
		
		track.streamer.setPacketizer(new H264Packetizer());
	...
	public void addAMRNBTrack(int audioSource, int destinationPort) {
		...
		int id = generateId();
		Track track = new Track(new MediaStreamer(),Track.AUDIO);
		
		track.streamer.setPacketizer(new AMRNBPacketizer());
		track.encoder = MediaRecorder.AudioEncoder.AMR_NB;
		track.format = MediaRecorder.OutputFormat.RAW_AMR;
		track.descriptor = "m=audio 5004 RTP/AVP 96\r\n" +
					"b=AS:128\r\n" +
					"b=RR:0\r\n" +
	...
	private void configureTrack(Track track) {
		...
		streamer.getPacketizer().setRtpSocket(new SmallRtpSocket(new byte[65536],destination,track.port));
		
		track.ssrc = new Random().nextInt();
		streamer.getPacketizer().getRtpSocket().setSSRC(track.ssrc);
